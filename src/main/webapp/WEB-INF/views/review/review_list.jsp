							...
							<tr>
							    ...
							    <td style="text-align: left;">
								    ...
								    <c:choose>
								    	...
								    	<c:otherwise>
								    		<a href="review_detail.do?re_idx=${k.re_idx}&cPage=${paging.nowPage}">${k.title }</a></td>
								    	</c:otherwise>
								    </c:choose>
							    <td>${k.writer }</td>
					...
					<td colspan="4">
						<ol class="paging">
							...
							<c:choose>
								...
								<c:otherwise>
									<li><a href="review_list.do?cPage=${paging.beginBlock - paging.pagePerBlock }">이전으로</a></li>
								</c:otherwise>
							</c:choose>
							<!-- 페이지번호들 -->
							<c:forEach begin="${paging.beginBlock }" end="${paging.endBlock }" step="1" var="k">
								<c:choose>
									...
									<c:otherwise>
										<li><a href="review_list.do?cPage=${k}">${k }</a></li>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							...
							<!-- 이후 버튼 -->
								...
								<c:otherwise>
									<li><a href="review_list.do?cPage=${paging.beginBlock + paging.pagePerBlock }">다음으로</a></li>
								</c:otherwise>
							</c:choose>
						</ol>	
					</td>
					<td>
						<input type="button" value="글쓰기" onclick="review_write()">
					</td>
				</tr>
			</tfoot>	
		</table>
