...
<style type="text/css">
.cart_list_table a:link, .cart_list_table a:visited{
	color: black;
}
</style>
<script
...
<script type="text/javascript">
    ...
    function select_delete(f) {
    	if (document.getElementById("cart_selTotal").textContent == 0 ) {
	...
	<div class="cart_list_wrapper">
				...
				<c:otherwise>
					<c:forEach var="k" items="${cartlist}" varStatus="vs">
						<tr>
							...
							<td><input type="checkbox" name="cart_check_idx" value="${k.cartlist_idx}" form="cartlist_selchk"/></td>
							<td class="img_show">
								
								<img style="border-radius: 10%;" src="resources/image/${k.product_img}" alt="향수" />
							</td>
							<td colspan="3"><a href="product_detail.do?product_idx=${k.product_idx}">${k.product_name}</a></td>
							<td>
								<form method="post">
									<input type="hidden" name="cartlist_idx"
										value="${k.cartlist_idx}" /> <input type="number"
										name="cartlist_count" min="1" value="${k.cartlist_count}"
										style="width: 50px" onchange="cartlist_edit(this.form)">
								</form>
							</td>
		...
		<form id="cartlist_selchk" method="post">
			<table class="cart_list_button">
				<tr>
					<th><input type="button" value="전체선택/해제"
						name="cart_checkbox_all" onclick="all_check(this.form)" /> <input
						type="button" value="선택삭제" onclick="select_delete(this.form)"></th>
					<th><b>선택상품금액 : <span id="cart_selTotal">0</span>
							원&nbsp;&nbsp;&nbsp;
					</b> <input type="button" value="선택구매" onclick="select_pay(this.form)"></th>
				</tr>
				<tbody>
					<tr class="cart_list_notice">
						<td>
							<p class="cart_list_notice_p">배송비용은 3,000원입니다.</p>
						</td>
					</tr>
				</tbody>
			</table>
